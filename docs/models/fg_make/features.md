...
| `shooter_make_c` | float32 | pbp `participant_1_id` via `shooter_form` | the shooter's expanding make rate on this class over his games strictly before this one, minus the league as-of rate | **exactly 0.0** when he has no prior attempt of the class | The first responsiveness driver. Rows where it is undefined are reported as their own cell, never bucketed on the imputation |
...
| `score_diff` | float32 | pbp running score | score from the SHOOTING team's point of view | — | |
