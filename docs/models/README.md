...
| L4 | Player attribution (which player is credited with the secondary stat after a team-level event) | [`attribution/`](attribution/model.md) | BAKE-OFF COMPLETE 2026-09-10 -- both folds (F1 selection, within-2025 robustness), all eight targets, full pre-registered grid (wall time 682.4s, no lever needed); 27 tests passing. 6 of 8 targets adopt a winner on F1 (`REB_off`/`REB_def`/`steal`/`block`/`assisted`/`blocked`); `assist` and `stolen` adopt NO WINNER on F1 (`assist` recovers a winner on the robustness fold). A `build_team_asof` construction bug (league as-of rate silently NaN/zero for all three binaries) was found and fixed first -- `docs/tests/attribution_team_asof_lg_defect_2026-09-10.md`. Full table: `attribution/model.md` section 4 |
