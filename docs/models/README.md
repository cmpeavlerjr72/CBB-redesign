...
| L3 | Field-goal make (made vs missed, three SEPARATE models by shot class) | [`fg_make/`](fg_make/model.md) | BAKE-OFF RUN 2026-09-10 -- `lgbm`/`C_plus_state` wins `FGA_rim` and `FGA_jump2` by 33.7 and 32.4 noise floors with both gates clear. `FGA_3` has only ONE passing arm, the team-level baseline, which is 92 floors WORSE on log loss and 116x too flat on the shooter driver: the binding gate is the 4-of-4 monotone rule on a defence driver whose true quintile span is 1.37 pp. The gate question is with the PM -- model.md section 4.3 |
