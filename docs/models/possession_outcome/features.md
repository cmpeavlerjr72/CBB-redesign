...
Two rules govern every row of the table below and are worth stating before it:
...
|---|---|---|---|---|---|
| `off_3pa_c` | float32 | `data/processed/possessions/possessions_{season}.parquet` | `100 * sum(fga_3) / sum(poss)` over the offence's earlier games this season, minus the same ratio over all earlier league games | `0.0` (= league mean) | three-point attempts per 100 possessions |
| `off_rim_c` | float32 | same | `100 * sum(fga_rim) / sum(fga)`, centred | `0.0` | rim share of field-goal attempts |
| `off_tov_c` | float32 | same | `100 * sum(tov) / sum(poss)`, centred; `tov` counts possessions with `terminal_event == 'TOV'` | `0.0` | turnover rate |
...
| `period` | float32 | `chances_{season}.parquet` | `period` | n/a | 1-2 regulation, 3+ overtime |
...
| Shot coordinates (`shot_location_x/y`) | 22-33% coverage before 2025 (`docs/tests/data_audit_cbbd_pbp_2026-09-10.md`). `docs/SIM_GUARDRAILS.md` section 4 requires rim-vs-jumper classification to come from the shot type, not coordinates, for backtest seasons -- which is what `shot_range` does. |
...
- The `FGA_rim` / `FGA_jump2` split of CONTINUATION chances is not comparable in season 2025: the rim
  share of putbacks is 53.9-55.0% in 2022-2024, 44.5% in 2025, and 56.3% in 2026, because ESPN moved
  `LayUpShot` putbacks into `TipShot` and, for that one season, into two-point `JumpShot`. See
  `docs/tests/possessions_build_2026-09-10.md` section 3.1. 2025 is the fold-2 TEST season, so this
  is a label defect upstream of every feature here and it blocks the `cont` model.
