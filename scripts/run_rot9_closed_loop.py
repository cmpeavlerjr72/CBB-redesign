#!/usr/bin/env python
"""
run_rot9_closed_loop.py -- the Decision-10 closed-loop check for the rotation
round-6 K1 arm AND the round-9 exit family (2026-09-11): a paired-stream engine run with the rotation's engine-produced state
LIVE against the same run with it FROZEN.

Pre-registration: `docs/models/rotation/experiments.md` section 14.9.

Round 6 adds NO margin, clock or foul term (14.9): `tau`, `P(k_in|size,k_out)`
and `logA` are fitted over all training waves pooled, so the L31 refit-without
instrument was already run on the objects that do carry state (round 5, 13.12)
and what this runner adds is the FREEZE -- the instrument that detects whether
the round-6 entry rule opens a NEW channel.

    .venv/Scripts/python.exe scripts/run_rot6_closed_loop.py --arm round6 --round6-arm T1
    .venv/Scripts/python.exe scripts/run_rot6_closed_loop.py --arm round6 --round6-arm T1 --freeze
    .venv/Scripts/python.exe scripts/run_rot6_closed_loop.py --grade
...
def run(args) -> int:
    ...
    assert_not_sealed([int(args.season)], context="rotation round 6 closed loop")
    ...
    if args.arm == "round6":
        flags["ENGINE_ROTATION_ARM"] = args.round6_arm
    if args.arm == "round9":
        flags["ENGINE_ROTATION_ARM"] = args.round9_arm
    for k, v in flags.items():
        os.environ[k] = str(v)
    ...
    if args.arm == "round6":
        from cbb_sim.engine import rotation_adapter as RA
        r6 = RA.load_round6(inp.games)
        prov = dict(r6["provenance6"])
        prov["arm"] = r6["arm"]
        prov["wave_provenance"] = r6["provenance"]
        prov["hazards"] = sorted(set(r6["hazard_sources"]))
        prov["comp"] = r6["comp_sources"]
        prov["tau"] = [float(x) for x in r6["TAU"]]
        print("  round-6 manifest:", json.dumps(prov, default=str), flush=True)
    if args.arm == "round9":
        from cbb_sim.engine import rotation_adapter as RA
        r9 = RA.load_round9(inp.games)
        prov = dict(r9["provenance9"])
        prov["exit_arm"] = r9["exit_arm"]
        prov["entry_arm"] = r9["arm"]
        prov["comp"] = r9["comp_sources"]
        prov["exit"] = r9["exit_sources"]
        prov["k_shrink"] = r9["k_shrink"]
        prov["wave_provenance"] = r9["provenance"]
        print("  round-9 manifest:", json.dumps(prov, default=str), flush=True)
    if args.arm == "round5":
        from cbb_sim.engine import rotation_adapter as RA
    ...
    meta = {
        ...
        "round": "rotation Decision-10 freeze, round-6 K1 and round-9 (experiments.md 21.15/22)",
        "created_at": datetime.now(UTC).isoformat(),
        "rotation_arm": args.arm, "round4_variant": args.round4_arm,
        "round5_variant": args.round5_arm, "round6_variant": args.round6_arm,
        "round9_variant": args.round9_arm, "refit_without_state": bool(args.nostate),
...
def main() -> int:
    ...
    ap.add_argument("--arm", default="round6",
                    choices=["reference", "round4", "round5", "round6", "round9"])
    ...
    ap.add_argument("--round6-arm", default="T1", choices=["T1", "K1", "A1"])
    ap.add_argument("--round9-arm", default="Z1", choices=["Z1", "Z2"])
    ...
    ap.add_argument("--tags", default="rot6_T1_live,rot6_T1_frozen")
    args = ap.parse_args()
    if args.grade:
        return grade(args)
    if not args.tag:
        nm = {"reference": "R2", "round4": args.round4_arm,
              "round5": args.round5_arm, "round6": args.round6_arm}[args.arm]
        sfx = "nostate" if args.nostate else ("frozen" if args.freeze else "live")
        args.tag = f"rot6_{nm}_{sfx}"
    return run(args)
