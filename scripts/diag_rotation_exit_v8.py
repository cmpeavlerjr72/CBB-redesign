#!/usr/bin/env python
"""
diag_rotation_exit_v8.py -- the round-8 report-only diagnostics (18.7).
...
sides, so every row is like for like:
  ...
  1. round 7's exit-side starter share (16.7), by swap size, so round 8's arms
     drop straight into 17.10's table;
  2. **the object round 8 exists to move**: the realised
     `P(a starter is the man who leaves | single swap, starters on the floor)`
     in the SIM against the same rate on the ACTUAL sequences -- 17.17's table
     with a simulated column.

`scripts/diag_rotation_exit_v7.py` supplies `_walk`, `_acc`, `_empty`, `_report`
and `wilson` unchanged; nothing in it is modified.

    .venv/Scripts/python.exe scripts/diag_rotation_exit_v8.py --sim-games 200
"""
...
OUT = ROOT / "data" / "processed" / "models" / "rotation" / "exit_audit_round8_2026-09-11.json"
ARMS = ["X1_exit_class", "Y1_exit_rate", "Y2_exit_rate_foul"]
