#!/usr/bin/env python
"""
run_rot4_closed_loop.py -- the Decision-10 closed-loop check for the rotation
layer: a paired-stream engine run with the rotation's engine-produced state
LIVE against the same run with it FROZEN.

Pre-registration: `docs/models/rotation/experiments.md` section 10.10.

    .venv/Scripts/python.exe scripts/run_rot4_closed_loop.py --arm reference
    .venv/Scripts/python.exe scripts/run_rot4_closed_loop.py --arm reference --freeze
    .venv/Scripts/python.exe scripts/run_rot4_closed_loop.py --arm round4
    .venv/Scripts/python.exe scripts/run_rot4_closed_loop.py --arm round4 --freeze
    .venv/Scripts/python.exe scripts/run_rot4_closed_loop.py --grade
...
def run(args) -> int:
    ...
    meta = {
        ...
        "round": "rotation round 4 (docs/models/rotation/experiments.md section 10.10)",
...
def grade(args) -> int:
    ...
    outp = res / "rot4_closed_loop_summary.json"
...
def main() -> int:
    ...
    ap.add_argument("--arm", default="reference", choices=["reference", "round4"])
    ...
    ap.add_argument("--tags", default="rot4_R2_live,rot4_R2_frozen,"
                                      "rot4_H1_live,rot4_H1_frozen")
    args = ap.parse_args()
    if args.grade:
        return grade(args)
    if not args.tag:
        nm = "R2" if args.arm == "reference" else args.round4_arm
        args.tag = f"rot4_{nm}_{'frozen' if args.freeze else 'live'}"
    return run(args)
