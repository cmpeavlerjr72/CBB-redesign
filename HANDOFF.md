...
Watch items added 2026-09-10 evening (from truth tables v1, `docs/tests/truth_tables_v1_2026-09-10.md`): (1) event-layer FTA/FTM vs box agree on only ~90-93% of team-games in every season, flat across the feed-completeness curve, pointing at `ft_trip_ambiguous` in the possessions layer rather than feed gaps; the free-throw model trains on those trips, so its S1 confirmation pass must report the ambiguous-trip share and re-check calibration on unambiguous trips. (2) Player crosswalk to CBBD ids is 0% for 2022-23 (no CBBD rosters pulled for those seasons); any player-level truth for those seasons is box-only. (3) 2025 has 4 flagged final-score disagreements between hoopR and CBBD, unresolved by design; grading excludes flagged games until a third source settles them.
