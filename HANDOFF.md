...
Written by the PM 12:20-13:00 EDT. Every number is in a committed doc; paths in brackets. Fifteen worker lanes ran (twelve by 12:45, three more by 13:10); all delivered.
...
2. Clock 5d: wire C4 (sigma^2 quadratic in pace) behind a flag, paired closed loop vs B1; then the between-game tempo slope; then L34 prev_end mix from the event side.
...
4. Rotation: write the round-7/8/9 engine adapter first (scope in experiments.md 21.15) so Decision 10 can run on K1 and Z1; then round 10 on the wave side conditioned on n_starters.
