...
## Decision 9: Every team and player rate feature is opponent-adjusted; the conference boundary is a modelled regime, not a calendar accident (user decision 2026-09-10)

**Decision:** (a) No sub-model consumes a raw as-of team or player rate as a quality signal. Every rate feature is expressed relative to its snapshot's league mean (existing rule) AND adjusted for the strength of the opponents it was accumulated against. The adjustment method is a bake-off arm (at minimum: raw-centred as the reference, one-pass opponent-mean adjustment, iterative adjustment to convergence), never assumed. (b) A conference-game flag is a first-class feature in every scoring-stage model, audited like home/away/neutral. (c) The in-season refit scheme's cadence and alignment are bake-off dimensions: calendar-monthly (the S1 definition adopted in L21), weekly, and conference-aligned (each team refits at its own first conference game, then on the standing cadence). Possession outcome runs this first; the other sub-models' S1 confirmation passes inherit its winner.
...
**Alternative considered:** Keep monthly S1 and rely on own_ratings (already opponent-adjusted) to carry quality while style rates stay raw-centred. Rejected by the user: the style rates themselves are the inflated quantities and the model reads them directly.
